...
static int count_mask_bits(const std::array<int, 69>& mask) {
    int count = 0;
...
static void test_builtin_dataset_and_enum_values() {
    auto cards = standardCards();
    CHECK(!cards.empty());
...
static void test_builtin_dataset_accessors_are_stable() {
    const auto& cards_a = standardCards();
    const auto& cards_b = standardCards();
    CHECK(cards_a.size() == cards_b.size());
    CHECK(!cards_a.empty());
    CHECK(cards_a.data() == cards_b.data());

    const auto& nobles_a = standardNobles();
    const auto& nobles_b = standardNobles();
    CHECK(nobles_a.size() == nobles_b.size());
    CHECK(!nobles_a.empty());
    CHECK(nobles_a.data() == nobles_b.data());
}
...
static void test_action_roundtrip_for_gem_moves() {
    ...
    for (const auto& m : samples) {
        int idx = moveToActionIndex(m);
        CHECK(idx >= 0 && idx < 69);
        Move decoded = actionIndexToMove(idx);
        CHECK(moveToActionIndex(decoded) == idx);
    }
}

static void test_action_roundtrip_for_choose_noble_actions() {
    GameState state;
    for (int noble_idx = 0; noble_idx < 3; ++noble_idx) {
        Move m;
        m.type = CHOOSE_NOBLE;
        m.noble_idx = noble_idx;
        int idx = moveToActionIndex(m);
        CHECK(idx == 66 + noble_idx);
        Move decoded = actionIndexToMove(idx);
        CHECK(decoded.type == CHOOSE_NOBLE);
        CHECK(decoded.noble_idx == noble_idx);
        CHECK(moveToActionIndex(decoded) == idx);
    }
}
...
static void test_reserve_faceup_refills_and_gives_joker() {
    ...
    CHECK(state.players[0].reserved[0].card.id == 10);
    CHECK(state.players[0].reserved[0].is_public == true);
...
static void test_reserve_faceup_no_joker_when_bank_empty() {
    ...
    CHECK(state.players[0].reserved.size() == 1);
    CHECK(state.players[0].reserved[0].card.id == 21);
    CHECK(state.players[0].reserved[0].is_public == true);
...
static void test_reserve_from_deck_does_not_touch_faceup() {
    ...
    CHECK(state.players[0].reserved.size() == 1);
    CHECK(state.players[0].reserved[0].card.id == 32);
    CHECK(state.players[0].reserved[0].is_public == false);
...
static void test_buy_reserved_card_updates_state_and_transfers_payment() {
    ...
    state.players[0].reserved.push_back(ReservedCard{reserved, true});
...
static void test_buy_with_two_claimable_nobles_enters_noble_choice_phase() {
    GameState state;
    clear_state(state);
    state.current_player = 0;
    ...
    CHECK(state.is_noble_choice_phase == true);
    CHECK(state.is_return_phase == false);
    CHECK(state.current_player == 0);
    CHECK(state.move_number == 0);
}

static void test_find_all_valid_moves_returns_only_choose_noble_in_noble_phase() {
    GameState state;
    clear_state(state);
    state.current_player = 0;
    state.is_noble_choice_phase = true;
    state.players[0].bonuses.blue = 1;
    ...
    CHECK(moves.size() == 2);
    for (const auto& m : moves) {
        CHECK(m.type == CHOOSE_NOBLE);
        CHECK(m.noble_idx == 0 || m.noble_idx == 1);
    }
}

static void test_choose_noble_claims_selected_noble_and_ends_turn() {
    GameState state;
    clear_state(state);
    state.current_player = 0;
    state.is_noble_choice_phase = true;
    ...
    Move choose;
    choose.type = CHOOSE_NOBLE;
    choose.noble_idx = 1;
    applyMove(state, choose);

    CHECK(state.players[0].nobles.size() == 1);
    CHECK(state.players[0].nobles[0].id == 42);
    CHECK(state.players[0].points == 3);
    CHECK(state.noble_count == 1);
    CHECK(state.available_nobles[0].id == 41);
    CHECK(state.is_noble_choice_phase == false);
    CHECK(state.current_player == 1);
    CHECK(state.move_number == 1);
}

static void test_apply_move_rejects_non_noble_actions_during_noble_choice_phase() {
    GameState state;
    clear_state(state);
    state.current_player = 0;
    state.is_noble_choice_phase = true;
...
static void test_is_game_over_during_noble_choice_phase_false_even_if_player_has_15() {
    GameState state;
    clear_state(state);
    state.is_noble_choice_phase = true;
    state.players[0].points = 15;
...
static void test_multiturn_reserve_return_then_buy_reserved_flow() {
    ...
    CHECK(state.players[0].reserved.size() == 1);
    CHECK(state.players[0].reserved[0].card.id == 70);
    CHECK(state.players[0].reserved[0].is_public == true);
    CHECK(state.players[0].tokens.joker == 1);
    CHECK(state.is_return_phase == true);
    CHECK(state.current_player == 0);
...
static void test_apply_move_throws_on_buy_reserved_out_of_range() {
    GameState state;
    clear_state(state);
    Move m;
    m.type = BUY_CARD;
...
static void test_apply_move_throws_on_reserve_from_empty_deck() {
    GameState state;
    clear_state(state);
    Move m;
    m.type = RESERVE_CARD;
...
static void test_apply_move_throws_on_invalid_return_gem_shape() {
    GameState state;
    clear_state(state);
    state.current_player = 0;
    state.is_return_phase = true;
    ...
    m.type = RETURN_GEM;
    m.gem_returned.white = 1;
    m.gem_returned.blue = 1; // invalid: two gems
...
static void test_apply_move_throws_on_unaffordable_buy() {
    GameState state;
    clear_state(state);
    state.current_player = 0;
    Tokens cost;
    cost.white = 2;
    state.faceup[0][0] = make_card(80, 1, 0, Color::Blue, cost);
    state.players[0].tokens.white = 1;
...
static void test_invalid_move_cases_throw_in_subprocess() {
    // TODO: This now locks in the safer exception contract for invalid direct applyMove() calls.
...
static void test_randomized_playouts_preserve_invariants_fixed_seed_set() {
    const unsigned int seeds[] = {1, 2, 3, 4, 5, 42, 123, 999, 2024, 7777};

    for (unsigned int seed : seeds) {
        GameState state;
        initializeGame(state, seed);
        for (int step = 0; step < 200 && !isGameOver(state); ++step) {
            auto moves = findAllValidMoves(state);
            CHECK(!moves.empty());
            auto mask = getValidMoveMask(state);
            for (const auto& m : moves) {
            ...
            size_t pick = (static_cast<size_t>(seed) + static_cast<size_t>(step) * 17u) % moves.size();
            applyMove(state, moves[pick]);
...
static void test_randomized_playouts_mask_and_apply_consistency_sampled() {
    const unsigned int seeds[] = {7, 11, 19, 23, 31};

    for (unsigned int seed : seeds) {
        GameState state;
        initializeGame(state, seed);
        for (int step = 0; step < 30 && !isGameOver(state); ++step) {
            auto moves = findAllValidMoves(state);
            CHECK(!moves.empty());
            auto mask = getValidMoveMask(state);
            ...
            size_t pick1 = (static_cast<size_t>(seed) + static_cast<size_t>(step) * 5u) % moves.size();
            int idx1 = moveToActionIndex(moves[pick1]);
            CHECK(moveToActionIndex(actionIndexToMove(idx1)) == idx1);

            if (moves.size() > 1) {
                size_t pick2 = (static_cast<size_t>(seed) + static_cast<size_t>(step) * 13u + 1u) % moves.size();
                int idx2 = moveToActionIndex(moves[pick2]);
                CHECK(moveToActionIndex(actionIndexToMove(idx2)) == idx2);
            }

            size_t apply_pick = (static_cast<size_t>(seed) + static_cast<size_t>(step) * 17u) % moves.size();
            applyMove(state, moves[apply_pick]);
...
int main() {
    ...
    test_builtin_dataset_and_enum_values();
    test_builtin_dataset_accessors_are_stable();
    test_find_all_valid_moves_initial_state();
    test_return_phase_move_generation();
    test_action_roundtrip_for_gem_moves();
    test_action_roundtrip_for_choose_noble_actions();
    ...
    test_buy_with_two_claimable_nobles_enters_noble_choice_phase();
    test_find_all_valid_moves_returns_only_choose_noble_in_noble_phase();
    test_choose_noble_claims_selected_noble_and_ends_turn();
    test_apply_move_rejects_non_noble_actions_during_noble_choice_phase();
    ...
    test_is_game_over_during_noble_choice_phase_false_even_if_player_has_15();
    ...
    test_apply_move_throws_on_buy_reserved_out_of_range();
    test_apply_move_throws_on_reserve_from_empty_deck();
    test_apply_move_throws_on_invalid_return_gem_shape();
    test_apply_move_throws_on_unaffordable_buy();
    test_invalid_move_cases_throw_in_subprocess();
    test_randomized_playouts_preserve_invariants_fixed_seed_set();
    test_randomized_playouts_mask_and_apply_consistency_sampled();
